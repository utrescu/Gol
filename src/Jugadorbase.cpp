...
JugadorBase::JugadorBase(Equip* equ, SDL_Rect casa, Punt3 mirant, double radi, Punt3 veloc, double massa,
			...
			double provaxut, double passades, double zconf, double dista, double espera, double dribling, jugo_de deq): 		
                    EntitatMobil(radi,veloc, maxvelocsense, mirant, massa, maxgir, maxforce), Nosaltres(equ),
...
bool JugadorBase::isPosicioDavantNostra(Punt3 position)const
{
	...
	if (ToSubject.Dot(getMirant()) > 0) return true;
	else  return false;
}
