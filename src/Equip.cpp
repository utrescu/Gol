...
Equip::Equip(char* nomfitxer, Porteria* nostra, Porteria* seva, Pantalla* pant, quin_equip costat, 
...
void Equip::CrearJugadors(char *nomfitxer, double Escala)
{
	...
	for (int i=0; i<numeroJugadors ; i++)
	{
		...
		if (i==0) sprintf(taulajugador, "Porter");
		     else sprintf(taulajugador, "Jugador%d",i);
...
void Equip::Pinta(SDL_Surface *p)
{
	...
	if (TincPilota())
	{
		...
		Posicio.w=20;
		Posicio.h=20;
		if (getQuinEquip()==EQUIP_LOCAL) SDL_FillRect(p,&Posicio,0x0000FF);
		                      else SDL_FillRect(p,&Posicio,0xFFFF40);
	}
