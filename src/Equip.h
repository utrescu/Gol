...
class Equip{
public:
	enum quin_equip {local, visitant};
private:
    ...
    //L'equip contrari
    Equip* Contraris;
    void CrearJugadors(char* nomfitxer, double Escala);
...
public:
	Equip(char* nomfitxer, Porteria* nostra, Porteria* seva, Pantalla* pant, quin_equip  costat, double Escala);
	...
	std::string getNomEquip()const{if (Soc == local) return "local"; return "visitant";}
	...
	bool  TincPilota()const{if(JugadorAmbLaPilota)return true; else return false;}
	void  PerdudaPilota(){JugadorAmbLaPilota = NULL; JugadorDeRecepcio=NULL; }
