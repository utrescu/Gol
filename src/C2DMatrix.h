...
//create a rotation matrix from a fwd and side 2D vector
	inline void  Rotate(const Punt3 &fwd, const Punt3 &side);

//applys a transformation matrix to a std::vector of points
	inline void TransformPunt3s(std::vector<Punt3> &vPoints);

//applys a transformation matrix to a point
	inline void TransformPunt3s(Punt3 &vPoint);
...
//applies a 2D transformation matrix to a std::vector of Punt3s
inline void C2DMatrix::TransformPunt3s(std::vector<Punt3> &vPoint)
{
...
//applies a 2D transformation matrix to a single Punt3
inline void C2DMatrix::TransformPunt3s(Punt3 &vPoint)
{
...
inline void C2DMatrix::Rotate(const Punt3 &fwd, const Punt3 &side)
{
	C2DMatrix::Matrix mat;
...
inline void VectorRodaPerOrigen(Punt3& v, double angle)
{
//	Punt3 temp = v;
//	temp.x = cos(angle)*v.x - sin(angle)*v.y;
//	temp.y = sin(angle)*v.x + cos(angle)*v.y;
//	v = temp;
  //create a transformation matrix
	C2DMatrix mat;
  //rotate
  ...
  //now transform the object's vertices
	mat.TransformPunt3s(v);
}

//--------------------- PointToLocalSpace --------------------------------
//
//------------------------------------------------------------------------
inline Punt3 PointToLocalSpace(const Punt3 &point,
                               const Punt3 &AgentHeading,
                               const Punt3 &AgentSide,
                               const Punt3 &AgentPosition)
{

	//make a copy of the point
  Punt3 TransPoint = point;
  ...
  matTransform.TransformPunt3s(TransPoint);
...
//------------------------------------------------------------------------
inline Punt3 VectorToWorldSpace(const Punt3 &vec,
                                const Punt3 &AgentHeading,
                                const Punt3 &AgentSide)
{
	//make a copy of the point
	...
	//now transform the vertices
	matTransform.TransformPunt3s(TransVec);
