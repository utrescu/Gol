...
void Pantalla::Carrega(SDL_Surface *pp, double Escala)
{
	...
	std::string nom = lm->getCadena("ImatgeCamp");
	// char *nom = lm->getCadena("ImatgeCamp");

	SDL_Surface *temp = IMG_Load(getFullFileName(nom).c_str());
	...
	// ---------------------------------  PORTERIES ---------------------------------------------------
	nom = lm->getCadena("ImatgePorteria");
...
void Pantalla::Posiciona(int x, int y)
{
	...
	if (ZonaImatge.x < 0)
		ZonaImatge.x = 0;
	if (ZonaImatge.x > MaxImatgeX)
		ZonaImatge.x = MaxImatgeX;

	if (ZonaImatge.y < 0)
		ZonaImatge.y = 0;
	if (ZonaImatge.y > MaxImatgeY)
		ZonaImatge.y = MaxImatgeY;
}
...
Punt3 Pantalla::getSacadaFora()
{
	...
	if (bimba->getPosicio().x > getTerrenyDeJoc()->getPuntMig().x)
		Quin = EQUIP_VISITANT;
	else
		Quin = EQUIP_LOCAL;
	if (bimba->getPosicio().y > getTerrenyDeJoc()->getPuntMig().y)
		Quin2 = 1;
	else
		Quin2 = 0;
